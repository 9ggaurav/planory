...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    let decodedToken;
    try {
        decodedToken = jwt.verify(incomingRefrehToken, process.env.JWT_REFRESH_SECRET);
    }
    catch (error) {
    ...
    if (user.refreshToken !== incomingRefrehToken) {
        throw new ApiError(401, "Refresh token does not match possibly user logged out or token is invalid");
    }
    const options = {
        httpOnly: true,
        secure: true,
...
export { registerUser, getUsers, loginUser, logoutUser, refreshAccessToken, changeCurrentPassword, getCurrentUser, updateAccountDetails, updateUserAvatar, getUserBoards, deleteUserById, };
