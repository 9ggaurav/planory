'use client';
import type { inboxTask as SharedInboxTask, Tasklist as tasklistType } from '@repo/shared';
import { Dialog, DialogContent, DialogTitle } from '@/components/ui/dialog';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { useTasks } from '@/providers/TaskContext';
import { useState } from 'react';
import MoreActions from '@/features/task/components/MoreActionsOnTask';
...
export default function DisplayTaskModal({ selectedTaskId }: childProp) {
  ...
  // const selectedTask: SharedInboxTask | undefined = (Tasks.find(
  //     task => task.id === selectedTaskId
  // ) as unknown) as SharedInboxTask | undefined;

  const selectedTask: SharedInboxTask | undefined = Tasks.find(
    task => String(task.id) === selectedTaskId,
  ) as unknown as SharedInboxTask | undefined;

  const currentTasklist: tasklistType | undefined = tasklist.find(
    list => list.id === selectedTask?.taskListId,
  );
  ...
  async function handleTaskCompletion(e: React.ChangeEvent<HTMLInputElement>) {
    if (!selectedTask) return;
    await updateTask(selectedTask.id, { isDone: e.target.checked });
  }
          ...
          <div className="px-5 pt-5 pb-0">
            ...
            <div className="flex items-start gap-3 mb-4">
              <label className="mt-0.75 shrink-0 cursor-pointer">
                ...
                />
                <div
                  className={`w-4.5 h-4.5 rounded-[5px] border-[1.5px] flex items-center justify-center transition-colors ${
                    selectedTask?.isDone
              ...
              {isEditingDescription ? (
                ...
                <h2
                  className={`text-[17px] font-medium leading-snug text-neutral-900 ${selectedTask?.isDone ? 'line-through text-neutral-400' : ''}`}
                >
                  {selectedTask?.title}
                </h2>
              )}
